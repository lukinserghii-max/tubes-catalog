...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main data-product-id={product.id} className="min-h-screen">
    <header className="border-b-2 border-[var(--ink)] bg-[var(--panel)]"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-5 py-4"><Link href="/" className="focus-ring flex items-center gap-3"><span className="grid h-10 w-10 place-items-center bg-[var(--signal)] text-xl font-black text-white">T</span><b className="display text-2xl tracking-[.08em]">Tubes AI</b></Link><div className="flex items-center gap-3"><a href={`/catalog.pdf#page=${product.page}`} target="_blank" rel="noreferrer" className="focus-ring border-2 border-[var(--ink)] px-3 py-2 text-sm font-bold hover:bg-[#e5e0d5]">PDF · стор. {product.page} ↗</a><Link href="/#catalog" className="focus-ring border-b-2 border-[var(--signal)] font-bold">← Каталог</Link></div></div></header>
