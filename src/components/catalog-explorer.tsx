...
type Catalog = {
  ...
  sections: { name: string; slug: string; count: number }[];
  products: Product[];
};
type Answer = {
  text: string;
  mode?: "ai" | "catalog";
  products?: { id: string; name: string; page: number }[];
    ...
    () =>
      [
        ...new Set(
          catalog.products
            .filter(
              (product) => section !== "all" && product.section === section,
            )
            .map((product) => product.categoryPath[1])
            .filter(Boolean),
        ),
      ].map((name) => ({
        name,
        count: catalog.products.filter(
          (product) =>
            product.section === section && product.categoryPath[1] === name,
        ).length,
      })),
      ...
      [
          ...
          catalog.products
            .filter(
              (product) =>
                ...
                product.categoryPath[1] === category,
            )
            .map((product) => product.categoryPath[2])
            .filter(Boolean),
        ),
      ].map((name) => ({
        name,
        count: catalog.products.filter(
          (product) =>
            product.section === section &&
            product.categoryPath[1] === category &&
            product.categoryPath[2] === name,
        ).length,
      })),
    [catalog.products, section, category],
  );
  const filtered = useMemo(() => {
    return catalog.products.filter((product) => {
      if (section !== "all" && product.section !== section) return false;
      if (category !== "all" && product.categoryPath[1] !== category)
        return false;
      if (subcategory !== "all" && product.categoryPath[2] !== subcategory)
        return false;
  ...
  return (
    <main>
        ...
        <div className="mb-7 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="relative w-full lg:w-[560px]">
            ...
            <input
              ...
              placeholder="Назва, індекс, призначення…"
              className="focus-ring h-14 w-full border-2 border-[var(--ink)] bg-[var(--panel)] pl-12 pr-4 text-lg shadow-[5px_5px_0_var(--ink)] placeholder:text-[var(--muted)]"
            />
          </div>
        </div>
        <div
      ...
      <section className="border-y-2 border-[var(--ink)] bg-[var(--signal)] text-white">
        <div className="mx-auto flex max-w-[1500px] flex-col gap-6 px-5 py-9 lg:flex-row lg:items-center lg:justify-between lg:px-10">
          <div>
            <h2 className="display text-4xl font-bold">
              Не знаєте, що обрати?
            </h2>
            <p className="mt-1 max-w-2xl text-white/85">
              Вкажіть речовину, тиск, температуру та діаметр. Експерт знайде
              кандидатів і чесно позначить, чого бракує для перевірки.
            </p>
          </div>
          <button
            onClick={() => setAssistantOpen(true)}
            className="focus-ring flex shrink-0 items-center justify-center gap-2 border-2 border-white bg-white px-6 py-3 font-bold text-[var(--signal)] hover:bg-[var(--ink)] hover:text-white"
          >
            <Icon name="spark" />
            Відкрити AI‑експерта
          </button>
        </div>
      </section>
      <footer className="bg-[var(--ink)] text-white">
        <div className="mx-auto flex max-w-[1500px] flex-col gap-4 px-5 py-8 text-sm text-white/65 sm:flex-row sm:items-center sm:justify-between lg:px-10">
          <span>
            Вітрина на основі українського каталогу TUBES International
          </span>
          <span>
            Підбір потребує підтвердження інженером та актуальною документацією
            виробника.
          </span>
        </div>
      </footer>
...
function ProductCard({ product, index }: { product: Product; index: number }) {
  return (
    <Link
      ...
      aria-label={`${product.name} — відкрити характеристики`}
    >
      <article
        className="rise paper-shadow flex h-full min-h-[390px] cursor-pointer flex-col border-2 border-[var(--ink)] bg-[var(--panel)] p-5 transition-transform duration-200 group-hover:-translate-y-1"
        style={{ animationDelay: `${Math.min(index, 8) * 35}ms` }}
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          <span className="border border-[var(--ink)] bg-[#dbe7e5] px-2 py-1 text-[10px] font-bold uppercase tracking-[.12em]">
            {product.sectionUk || product.section}
          </span>
          <span className="display text-sm text-[var(--muted)]">
            PDF / {product.page}
          </span>
        </div>
        <div className="mb-5 grid h-40 place-items-center overflow-hidden border border-[var(--line)] bg-white">
          {product.image ? (
            <img
              src={product.image}
              alt={`Фото продукції: ${product.name}`}
              loading="lazy"
        ...
        <h3 className="display text-3xl font-bold leading-none tracking-tight transition-colors group-hover:text-[var(--signal)]">
          {product.name}
        </h3>
        <p className="mt-3 line-clamp-3 text-sm leading-relaxed text-[var(--muted)]">
          {product.excerptUk ||
            product.excerpt ||
            "Опис і параметри товару знаходяться на сторінці каталогу."}
        </p>
        <div className="mt-auto flex items-center justify-between border-t border-[var(--line)] pt-4 font-bold">
          <span>Характеристики · {product.variants.length} інд.</span>
          <span className="grid h-9 w-9 place-items-center border-2 border-[var(--ink)] bg-[var(--ink)] text-white transition-transform group-hover:translate-x-1">
            <Icon name="arrow" />
          </span>
        </div>
      </article>
...
function Assistant({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setAnswer({
        text: "Не удалось связаться с экспертом. Попробуйте ещё раз.",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <aside className="ml-auto flex h-full w-full max-w-2xl flex-col bg-[var(--panel)] shadow-2xl">
        <div className="flex items-center justify-between border-b-2 border-[var(--ink)] bg-[var(--steel)] p-5 text-white">
          <div>
            <p className="text-xs uppercase tracking-[.2em] text-white/70">
              TUBES AI
            </p>
            <h2 id="assistant-title" className="display text-3xl font-bold">
              Експерт каталогу
            </h2>
          </div>
          <button
            onClick={onClose}
            className="focus-ring grid h-11 w-11 place-items-center border-2 border-white text-2xl"
            aria-label="Закрити"
          >
            ×
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-7">
          <div className="border-l-4 border-[var(--signal)] bg-[#f1e7dc] p-4 text-sm leading-relaxed">
            <b>Єдине джерело знань — PDF-каталог TUBES International.</b> Для
            точного пошуку вкажіть речовину, концентрацію, температуру, тиск,
            внутрішній діаметр і режим роботи. Якщо каталог не містить
            відповіді, експерт не буде її вигадувати.
          </div>
          {answer ? (
            <div className="mt-6 space-y-5">
              <div className="inline-flex border border-[var(--steel)] px-2 py-1 text-xs font-bold uppercase tracking-wider text-[var(--steel)]">
                {answer.mode === "ai"
                  ? "AI-аналіз каталогу"
                  : "Локальний пошук без AI-моделі"}
              </div>
              ...
              {answer.products?.length ? (
                <div>
                  <b className="display text-xl">Знайдені позиції</b>
                  <div className="mt-2 grid gap-2">
                    {answer.products.map((product) => (
                      <Link
                        ...
                        key={product.id}
                        href={`/product/${product.id}`}
                        className="focus-ring flex justify-between border border-[var(--line)] p-3 hover:border-[var(--signal)]"
                      >
              ...
              {answer.sources?.length ? (
                <div>
                  <b className="display text-xl">Джерела в каталозі</b>
            ...
            <div className="mt-10 text-center text-[var(--muted)]">
              <span className="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-full border-2 border-[var(--steel)] text-[var(--steel)]">
                <Icon name="book" />
              </span>
              <p>
                Наприклад: «Потрібен шланг для 30% розчину сірчаної кислоти,
                40°C, 6 бар»
              </p>
            </div>
          )}
        ...
        <form
          onSubmit={submit}
          className="border-t-2 border-[var(--ink)] p-4 sm:p-5"
        >
          ...
          <textarea
            ...
            rows={3}
            placeholder="Опишіть середовище та умови роботи…"
            className="focus-ring w-full resize-none border-2 border-[var(--ink)] bg-white p-3"
          />
          <button
            disabled={loading || !message.trim()}
            className="focus-ring mt-3 flex w-full items-center justify-center gap-2 bg-[var(--signal)] px-5 py-3 font-bold text-white disabled:opacity-50"
          >
            {loading ? "Перевіряю каталог…" : "Отримати відповідь"}
            <Icon name="arrow" />
          </button>
        </form>
      </aside>
